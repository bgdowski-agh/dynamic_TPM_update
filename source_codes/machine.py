from update_rules import hebbian, anti_hebbian, random_walk, dynamic_matrix, dynamic_row, random_eve
import numpy as np
...
class Machine:
	...
	'''
	def __init__(self, k=3, n=4, l=6, W=np.random.randint(-1,1,[1,1])):
		'''
		Arguments:
		k - The number of hidden neurons
		n - Then number of input neurons connected to each hidden neuron
	...
	def update(self, tau2, update_rule='hebbian'):
		...
		if (tau1 == tau2):
			...
			elif update_rule == 'dynamic_row':
				dynamic_row(W, X, sigma, tau1, tau2, l)
			elif update_rule == 'dynamic_matrix':
				dynamic_matrix(W, X, sigma, tau1, tau2, l)
			elif update_rule == 'random_eve':
				random_eve(W, X, sigma, tau1, tau2, l)
			else:
